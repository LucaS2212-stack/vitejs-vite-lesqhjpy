export default async function handler(req, res) {
  try {
    ...
    // Open Food Facts PRIMA
    try {
      const offRes = await fetch(`https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(query)}&search_simple=1&action=process&json=1&page_size=10&lc=it&cc=it`, {
      ...
      (offData.products || []).forEach(p => {
        ...
        const cal = Math.round(n["energy-kcal_100g"] || n["energy-kcal"] || (n["energy-kj_100g"] ? n["energy-kj_100g"] / 4.184 : 0) || 0);
