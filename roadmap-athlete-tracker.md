...
|---|---|---|---|---|---|
| D1 | 🔴 | ⬜ | Font "Inter Tight" mai caricato | L'unico `@import` font nel progetto (fondo di `App.jsx`) carica solo `Inter`. `index.html` non ha nessun `<link>` a Google Fonts. Tutti i numeri grandi/titoli che usano `fTight` stanno ripiegando su Inter normale o system-ui. Contraddice la decisione 0. | `App.jsx:2147`, `index.html` |
...
| D4 | 🟡 | ⬜ | `Kicker` (badge quadratino+rosso) assente in metà dei tab | Presente in Dashboard, Peso, Check-in. Assente in Oggi, Piano, Planning, Meal Plan — questi tab partono dritti con le card, senza l'intestazione di sezione che il resto dell'app usa. | tab `oggi` (1364+), `piano` (1621+), `planning` (1741+), `meal` (2036+) |
...
| D7 | 🟢 | ⬜ | `index.html` non rinominato | `<title>Vite + React + TS</title>`, `lang="en"` — mai personalizzato, si vede nella tab del browser. | `index.html:2-7` |
...
| # | Priorità | Stato | Voce | Descrizione | Riferimento |
|---|---|---|---|---|---|
| I1 | 🔴 | 🔧 | Questa cartella locale non è un repository git | Nessuna cartella `.git` da nessuna parte. Il nome `vitejs-vite-lesqhjpy-main` è tipico di uno "Download ZIP" da GitHub. **Se il deploy Vercel è collegato a un repo GitHub reale**, questa copia locale non è quel repo — modifiche fatte qui non arrivano in produzione finché non colleghi/pushi manualmente. **2026-07-16**: l'utente conferma di avere il link del repo reale, in attesa che lo fornisca per procedere al collegamento. | root del progetto |
| I2 | 🔴 | 🔧 | Verificare le policy RLS su tutte le tabelle Supabase | L'anon key è pubblica nel bundle JS (normale per Supabase), ma la sicurezza dipende interamente da RLS lato Supabase. Da controllare esplicitamente per: `athlete_days`, `athlete_weight`, `athlete_plan_history`, `athlete_planning`, `athlete_meal_plan`, `athlete_foods`, `athlete_week_checkin`. Se anche una sola tabella non ha RLS attiva con filtro su `user_id = auth.uid()`, chiunque può leggere/scrivere dati di altri utenti. **2026-07-16**: l'utente ha chiesto una guida passo-passo, vedi messaggio in conversazione / sezione 6. | `App.jsx:6-8` |
